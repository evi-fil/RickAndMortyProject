<template>
	<div class="flex justify-center">
		<button @click="navigateHome" class="group mt-4 bg-lime-400 text-fuchsia-800 hover:ring hover:ring-lime-500 hover:shadow-md hover:shadow-lime-500 
			hover:bg-fuchsia-800 hover:text-lime-400 font-semibold text-3xl w-16 h-10 lg:w-20 rounded-full 
...
const navigateHome = () => {
	if (store.searchActive) {
		...
		router.push({ path: '/all-characters', query: { page: currentPage } });
	}
};
</script>